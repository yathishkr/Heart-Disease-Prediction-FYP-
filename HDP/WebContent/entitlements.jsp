			...
			<div class="container navigation">
				...
				<div
					...
					<ul class="nav navbar-nav">
						...
						<li class="active"><a href="entitlements.jsp">Entitlement</a></li>
					...
					<div class="col-lg-8 col-lg-offset-2">
						<div class="wow lightSpeedIn" data-wow-delay="0.1s">
							<div class="section-heading text-center">
								...
								<p>Here, you can request the Admin to get access to our Prediction Model</p>
							</div>
						</div>
					</div>
				</div>
				<div class='row'>
					<div class="col-lg-8 col-lg-offset-2">
						...
						<%
							...
							Entitlement ent = eDao.getByEmail(u1.getEmail());
							if (ent == null || ent.getEmail() == null)
							{
							   %>
							   		<hr/>
							   		<h6> Your Entitlement Request to access the Disease Predictor Model isn't started yet.</h6>
							   		<hr/>
							   		<form action='entitlement'>
							...
							{							   
							   if (ent.getStatus().equals("APPROVED"))
							   {
								%>
							      		<hr/>
							      		<h6>Your Entitlement Request has been <b style='color: lightgreen;'> APPROVED </b> by <b><%=ent.getApprover() %></b></h6>
							      		<hr/>
								   		<form action='entitlement'>
								   			<input type=hidden name='req_type' value='cancel' />
								   			<input class='btn btn-red' type=submit value='Terminate Entitlement' />
								   		</form>
								   		<hr/>
								
								<%						      
							   }
							   else if (ent.getStatus().equals("REJECTED"))
							   {
							      %>
							      		<hr/>
							      		<h6>Your Entitlement Request has been <b style='color: red;'> REJECTED </b> by <b><%=ent.getApprover() %></b></h6>
							      		<hr/>
								   		<form action='entitlement'>
								   			<input type=hidden name='req_type' value='cancel' />
								   			<input class='btn btn-red' type=submit value='Cancel Request' />
								   		</form>
								   		<hr/>
							   ...
							   else
							   {
							      %>
							      		<hr/>
							      		<h6>Your Entitlement Request has been Submitted on <b><%=sdf1.format(ent.getEntrytime()) %></b> at <b><%=sdf2.format(ent.getEntrytime()) %></b></h6>
							      		<hr/>
							      		<h6>Your Request is still under Review.</h6>
							      		<hr/>
								   		<form action='entitlement'>
								   			<input type=hidden name='req_type' value='cancel' />
								   			<input class='btn btn-red' type=submit value='Cancel Request' />
